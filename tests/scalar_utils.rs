...
mod tests {
    ...
    fn test_shl_shr() {
        ...
        for _ in 0..100 {
            ...
            let mut b_arr = ScalarBits::from_scalar(&r, 253);
    ...
    fn test_scalar_to_u64_array() {
...

        {
            ...
            println!("s={:?}", s);
            let b = ScalarBits::from_scalar(&s, 253);
            println!("b={:?}", b);
            let s1 = b.to_scalar();
            println!("s1={:?}", s1);
    ...
    fn test_hex() {
        let h: &str = "6a09e667";
        let s: u32 = u32::from_str_radix(h, 16).unwrap();
        println!("{:?}", Scalar::from(s));
